...
public class UIManager : BaseManager {
    ...
    {
        //使用Unity主线程去加载Panel，如果这个变量不为None就调用GameFacade中的UpdateManager方法
        if (panelTypeToPush != UIPanelType.None)
        {
            PushPanel(panelTypeToPush);
            panelTypeToPush = UIPanelType.None;//push完要归None
        }
    }
    ...
    private MessagePanel messagePanel;//方便其他面板调用显示信息面板
